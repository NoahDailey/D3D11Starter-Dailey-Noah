...
Mesh::Mesh(Vertex vertices[], unsigned int numberOfVertices, unsigned int indices[], unsigned int numberOfIndices)
{
	...
	//    be if we want the GPU to act on it (as in: draw it to the screen)
	{
		...
		vbd.ByteWidth = sizeof(Vertex) * totalVertices;       // Size of a vertex times the number of vertices in the buffer
	...
	//    be if we want the GPU to act on it (as in: draw it to the screen)
	{
		...
		ibd.ByteWidth = sizeof(unsigned int) * totalIndices;	// Size of an unsigned int times the number of indices in the buffer
