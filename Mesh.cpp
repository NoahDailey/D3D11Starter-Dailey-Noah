...
Mesh::Mesh(Vertex* vertices, unsigned int verticesCount, unsigned int* indices, unsigned int indicesCount, char* name)
{
	...
	//    be if we want the GPU to act on it (as in: draw it to the screen)
	{
		...
		vbd.ByteWidth = sizeof(Vertex) * this->verticesCount;       // 3 = number of vertices in the buffer
	...
	//    be if we want the GPU to act on it (as in: draw it to the screen)
	{
		...
		ibd.ByteWidth = sizeof(unsigned int) * this->indicesCount;	// Size of an intnumber of indices in the buffer
